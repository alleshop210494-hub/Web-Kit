...
import {
  ...
  X, Download, Upload, TrendingUp, Truck, Printer, CheckSquare, Pencil
} from 'lucide-react'
...
export const Dashboard = () => {
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 13%;" class="text-right">Harga Total</th>
              ${customHeadersHTML}
          ...
          <tbody>
            ${allUserItems.map((item, index) => {
              const itemTotal = (item.stock || 0) * (item.price || 0)
  ...
  const handleExportCSV = () => {
    let headers = ['ID', 'SKU', 'Nama Barang', 'Kategori', 'Stok', 'Harga Satuan (Rp)', 'Harga Total (Rp)', ...customColumns, 'Lokasi Rak', 'Supplier']
    let csvRows = [headers.join(';')]
    allUserItems.forEach(item => {
      let itemTotal = (item.stock || 0) * (item.price || 0)
